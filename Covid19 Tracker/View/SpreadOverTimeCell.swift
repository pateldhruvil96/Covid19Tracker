...
final class SpreadOverTimeCell: UICollectionViewCell {
    ...
    func setup(historicalTimelineList: [CountryDetails]) {
        let firstDate = historicalTimelineList.first?.date.formateDate()
        let lasteDate = historicalTimelineList.last?.date.formateDate()
        
        subtitleLabel.text = "(\(firstDate ?? "") - \(lasteDate ?? ""))"
    ...
    func setupBarChart(list: [CountryDetails]) {
        ...
        list.enumerated().forEach { (index, info) in
            ...
            let confirmedEntry = BarChartDataEntry(x: Double(index), y: Double(info.confirmed))
